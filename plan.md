...
| Method | Path | 説明 |
|---|---|---|
| GET | `/api/images` | 一覧取得（ページネーション、ソート対応） |
| GET | `/api/images/:id` | 詳細取得 |
| GET | `/api/images/search?q=...` | FTS5全文検索 |
...
| POST | `/api/images/:id/tags` | 画像にタグ付与 |
...
export const exifFieldMapping = {
  prompt: 'UserComment',   // 実際のフィールド名に合わせて変更
  title: 'XPTitle',        // 実際のフィールド名に合わせて変更
} as const;
```
...
| パス | 画面 | 状態管理 |
|---|---|---|
| `/` | 画像一覧 | 検索・フィルタはReactステートで保持（URLに載せない） |
| `/images/:id` | 画像詳細 | IDだけURLに載せる。戻るボタンで一覧に帰れる |

検索条件やタグフィルタをURLに反映させるのは、必要になってから足す。
...
- [ ] プロジェクト初期化（Bun + Hono + Vite + React）
- [ ] ESLint + Prettier + tsconfig 厳格設定
- [ ] Branded Types + zodスキーマ定義（shared/brands.ts, schemas.ts）
- [ ] 環境変数によるパス解決（config.ts）
- [ ] SQLiteスキーマ構築（マイグレーション）
- [ ] 単一ファイルインジェスト（EXIF読取 + サムネイル生成）
- [ ] フルスキャン同期（更新ボタンから手動トリガー）
- [ ] 画像一覧API + グリッド表示（仮想スクロール）
- [ ] プロンプト全文検索
...
- [ ] タグCRUD + 画像へのタグ付与UI
- [ ] タグでのフィルタ（複数タグのAND/OR切替）
- [ ] バルクタグ付け（複数画像を選択して一括付与）
...
設計過程で検討し、採用または却下した判断の記録。
Claude Codeが「なぜこうなっているか」を理解し、同じ議論を繰り返さないためのもの。

### ADR-001: 言語選定 — 全部TSに統一、Rustハイブリッドは見送り

**ステータス:** 採用  
**日付:** 2026-03-19

**背景:** インジェスト処理（数千枚のEXIF読取＋サムネイル生成）をRust CLIで切り出し、バックエンド/フロントをTSで書くハイブリッド構成も検討した。

**決定:** 全部TSに統一する。

**理由:**
- このアプリはI/Oバウンド中心で、Rustの速度が活きる場面が少ない
- ビルドチェーンが分かれるとClaude Codeの自律開発が複雑になる
- exifr, sharp など TS エコシステムで十分カバーできる
- 一つのコードベースで完結する方がメンテコストが低い

**却下した案:** Rust CLI（ingest） + TS（バックエンド/フロント）のハイブリッド

---

### ADR-002: ORM — Prisma不採用、bun:sqlite直接利用

**ステータス:** 採用  
**日付:** 2026-03-19

**背景:** DBアクセス層にPrismaを入れるか検討した。

**決定:** Prismaは使わない。bun:sqliteのネイティブバインディングを直接使い、queries.tsに型付きヘルパーを薄く書く。

**理由:**
- FTS5（CREATE VIRTUAL TABLE、トリガー）はPrismaのスキーマ定義で表現できず、結局生SQLを書くことになる
- bun:sqliteとの間に余計なレイヤーが入り、パフォーマンスと透明性が下がる
- スキーマ変更のたびに `prisma generate` → `prisma migrate` のステップが必要で、Claude Codeの自律開発フローに摩擦が生じる
- この規模ならORMの抽象化の恩恵より、SQLを直接制御できるメリットの方が大きい

**却下した案:** Prisma + bun:sqlite

---

### ADR-003: お気に入り機能 — 専用カラム廃止、タグシステムに統合

**ステータス:** 採用  
**日付:** 2026-03-19

**背景:** 当初 `is_favorite` カラムを images テーブルに持つ設計だった。

**決定:** お気に入り専用のカラム・API・コンポーネントを全廃し、タグシステムに統合する。予約タグも設けず、完全にユーザー任せとする。

**理由:**
- お気に入りは「特別なタグ」でしかない。概念として重複している
- タグに統合すれば、お気に入り以外にも「要確認」「ボツ」等の運用タグを同じ仕組みで回せる
- 専用カラム分のスキーマ・API・UIコンポーネントが減り、コードベースがシンプルになる
- 予約タグを入れるとシステム側の都合がユーザーの自由度を制約する

**却下した案:** is_favorite カラム + 専用API + FavoriteToggle コンポーネント

---

### ADR-004: ファイル同期 — ファイルウォッチャー不採用、手動更新ボタン方式

**ステータス:** 採用  
**日付:** 2026-03-19

**背景:** 画像ディレクトリの変更（追加・削除・上書き・メタデータ更新）を検知する方法として、chokidar等のファイルウォッチャーによるリアルタイム監視と、手動トリガーのフルスキャンを検討した。当初はウォッチャー＋起動時フルスキャンのハイブリッドで設計していた。

**決定:** ファイルウォッチャーは使わない。UIの「更新」ボタンを押した時にフルスキャンを実行する。

**理由:**
- ウォッチャーが落ちている間の変更を取りこぼすリスクがあり、結局フルスキャンも必要になる
- シングルユーザーで、画像追加のタイミングは自分でわかっている
- chokidarの依存を削除でき、サーバー側の常駐プロセスも不要になる
- シンプルさを優先する

**却下した案:** chokidar によるリアルタイム監視 + 起動時フルスキャン

---

### ADR-005: 削除方針 — ソフトデリート不採用、物理DELETE

**ステータス:** 採用  
**日付:** 2026-03-19

**背景:** ファイルシステムから消えた画像のDB上の扱いとして、`deleted_at` カラムによるソフトデリートを検討した。タグやお気に入りメタデータの保護が動機だった。

**決定:** 物理DELETEを使う。`ON DELETE CASCADE` でタグ関連も自動削除する。

**理由:**
- DBには画像のメタデータしか入っていない。画像ファイルが唯一のソースであり、ファイルが消えたらメタデータだけ残しても意味がない
- お気に入りがタグに統合されたことで、保護すべき独自メタデータの価値が更に下がった
- ソフトデリートは `WHERE deleted_at IS NULL` を全クエリに付ける必要があり、バグの温床になる
- パージ処理等の運用負担も増える

**却下した案:** `deleted_at` カラムによるソフトデリート + 将来のパージ処理

---

### ADR-006: フロントエンドホスティング — 外部ホスティング不採用、コンテナ同梱

**ステータス:** 採用  
**日付:** 2026-03-19

**背景:** フロントエンドだけCloudflare等でホスティングし、ローカルのバックエンドと通信させる構成を検討した。

**決定:** フロントエンドはバックエンドと同一コンテナから配信する。

**理由:**
- ローカルAPIをインターネットに公開するセキュリティリスクが割に合わない
- HTTPS（Cloudflare）→ HTTP（localhost）の mixed content がブラウザにブロックされる。回避策はどれも複雑
- ローカルでサーバーを起動する時点でフロントも一緒に配信すれば済む
- シングルユーザー・ローカル用途ではメリットが薄い

**却下した案:** Cloudflare Pages（フロント） + Cloudflare Tunnel or ポート開放（バックエンド通信）

---

### ADR-007: ルーティング — ライブラリ不採用、最小限のURL設計

**ステータス:** 採用  
**日付:** 2026-03-19

**背景:** React Router や TanStack Router の導入を検討した。また、検索・フィルタ条件をURLクエリパラメータに反映する設計も検討した。

**決定:** ルーティングライブラリは使わない。`history.pushState` + `popstate` の薄いラッパーのみ。URLは `/`（一覧）と `/images/:id`（詳細）の2本だけ。検索・フィルタ状態はReactステートで持ち、URLには載せない。

**理由:**
- ローカル専用で、URLのブックマークや共有の需要がない
- 画像詳細からの「戻る」だけ効けば十分
- 検索・フィルタのURL反映は、必要になってから足しても遅くない（YAGNI）
- 依存を増やさずに済む

**却下した案:** React Router / TanStack Router、検索・タグフィルタのURLクエリパラメータ化

---

### ADR-008: EXIFフィールドマッピング — フォールバック不要、単一フィールド固定

**ステータス:** 採用  
**日付:** 2026-03-19

**背景:** 当初、複数の生成AIツール（Stable Diffusion, ComfyUI, Midjourney等）に対応するため、EXIFフィールドの優先順位付きフォールバックリストを設計していた。

**決定:** フォールバック不要。config.tsにフィールド名を1つずつ指定するだけ。

**理由:**
- 画像生成は自作ツールで行っており、メタデータの格納先が固定されている
- フォールバックロジックは使われないコードパスを増やし、テスト・デバッグの負担になる
- 将来ツールを変更した場合も config.ts を書き換えるだけで対応できる

**却下した案:** UserComment → ImageDescription → XPComment → parameters の優先順位付きフォールバック

...
- 比較モードはURLルーティングではなくコンポーネントstateで管理する（URLに反映しない一時的な表示のため）
...
**決定:** ルーティングライブラリは使わない。`history.pushState` + `popstate` の薄いラッパーのみ。URLは `/`（一覧）と `/images/:id`（詳細）の2本だけ。検索・フィルタ状態はReactステートで持ち、URLには載せない。比較モードはURLルーティングではなくコンポーネントstateで管理する（URLに反映しない一時的な表示のため）。
